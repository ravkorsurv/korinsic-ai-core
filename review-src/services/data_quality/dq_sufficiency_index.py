...
from .dq_strategy_base import DQConfig, DQScoringStrategy, DQSIOutput, KDEResult
...
class DataQualitySufficiencyIndex:
    ...
    def __init__(self, config: Optional[DQConfig] = None):
        ...
        logger.info(
            f"DQSI engine initialized with strategy: {self.strategy.get_strategy_name()}"
        )
    ...
    def _create_strategy(self) -> DQScoringStrategy:
        ...
        if strategy_name == "fallback":
            ...
            logger.warning(
                f"Unknown strategy '{strategy_name}', falling back to 'fallback' strategy"
            )
            return FallbackDQScoringStrategy(self.config)
    ...
    def calculate_dqsi(
        ...
        try:
            ...
            confidence_index, confidence_note = (
                self.strategy.calculate_confidence_index(kde_results, dqsi_score)
            )
            ...
            kde_weights = {kde.kde_name: kde.risk_weight for kde in kde_results}
            ...
            logger.info(
                f"DQSI calculated: score={dqsi_score:.3f}, confidence={confidence_index:.3f}"
            )
    ...
    def calculate_dqsi_for_case(self, case_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if all_kde_results:
            ...
            confidence_index, confidence_note = (
                self.strategy.calculate_confidence_index(all_kde_results, dqsi_score)
            )
            sub_scores = self.strategy.calculate_dimension_scores(all_kde_results)

            critical_kdes_missing = [
                kde.kde_name
                for kde in all_kde_results
                if kde.kde_name in self.config.critical_kdes and kde.score == 0.0
            ]

            kde_weights = {kde.kde_name: kde.risk_weight for kde in all_kde_results}
        else:
            dqsi_score = 0.0
    ...
    def validate_kde_coverage(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        coverage_ratio = (
            len(present_kdes) / len(expected_kdes) if expected_kdes else 0.0
        )
        ...
        return {
            ...
            "critical_missing": [
                kde for kde in missing_kdes if kde in self.config.critical_kdes
            ],
        }

    def simulate_dqsi_impact(
        self, data: Dict[str, Any], kde_modifications: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        confidence_delta = (
            modified_output.dqsi_confidence_index
            - original_output.dqsi_confidence_index
        )
    ...
    def _estimate_improvement_impact(self, kde_result: KDEResult) -> float:
        ...
        normalized_impact = (
            impact / max_possible_impact if max_possible_impact > 0 else 0.0
        )
