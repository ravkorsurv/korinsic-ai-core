...
class KDEFirstDQCalculator:
    ...
    def calculate_dqsi(
        ...
        try:
            ...
            for kde_name in applicable_kdes:
                ...
                kde_scores[kde_name] = self._calculate_kde_scores(
                    kde_name, kde_value, evidence
                )

            # Calculate synthetic KDE scores (timeliness, coverage)
            synthetic_scores["timeliness"] = self._calculate_timeliness_score(
                evidence, alert_timestamp
            )
            synthetic_scores["coverage"] = self._calculate_coverage_score(
                evidence, baseline_data
            )

            # Aggregate final DQSI score using weighted average
            dqsi_score, score_breakdown = self._aggregate_dqsi_score(
                kde_scores, synthetic_scores
            )
            ...
            legacy_score = (
                self._calculate_legacy_score(kde_scores, synthetic_scores)
                if self.config.get("legacy_support", {}).get("enabled", False)
                else None
            )
            ...
            result = {
                ...
                "user_role": user_role,
                "kde_scores": {
                    k: {dim: round(v, 3) for dim, v in scores.items()}
                    for k, scores in kde_scores.items()
                },
                "synthetic_scores": {
                    k: round(v, 3) for k, v in synthetic_scores.items()
                },
                "score_breakdown": score_breakdown,
                "applicable_kdes": applicable_kdes,
                "dimension_summary": self._get_dimension_summary(
                    kde_scores, synthetic_scores
                ),
                "quality_metadata": {
                    "total_kdes_assessed": len(applicable_kdes),
                    "synthetic_kdes_count": len(synthetic_scores),
                    "foundational_score": score_breakdown.get(
                        "foundational_weighted_score", 0.0
                    ),
                    "enhanced_score": score_breakdown.get(
            ...
            logger.info(
                f"KDE-First DQSI calculated: score={dqsi_score:.3f}, "
    ...
    def _get_applicable_kdes(
        self, evidence: Dict[str, Any], user_role: str
    ) -> List[str]:
        ...
        logger.debug(
            f"Role {user_role}: {len(applicable)} applicable KDEs out of {len(role_scope)} in scope"
        )
        return applicable

    def _calculate_kde_scores(
        self, kde_name: str, kde_value: Any, evidence: Dict[str, Any]
    ) -> Dict[str, float]:
        """
    ...
    def _score_completeness(self, value: Any) -> float:
        ...
        if isinstance(value, str) and (
            value == "" or value.lower() in ["null", "none", "unknown", "n/a"]
        ):
            return 0.0
    ...
    def _score_accuracy(self, kde_name: str, value: Any) -> float:
        ...
        precision_score = self._score_precision(
            kde_name, value, accuracy_rules.get("precision_rules", {})
        )
        validity_score = self._score_validity(
            kde_name, value, accuracy_rules.get("validity_rules", {})
        )
    ...
    def _score_precision(
        self, kde_name: str, value: Any, precision_rules: Dict[str, Any]
    ) -> float:
    ...
    def _score_validity(
        self, kde_name: str, value: Any, validity_rules: Dict[str, Any]
    ) -> float:
    ...
    def _score_uniqueness(
        self, kde_name: str, value: Any, evidence: Dict[str, Any]
    ) -> float:
    ...
    def _score_consistency(
        ...
        for source_name, source_config in golden_sources.items():
            if kde_name in source_config.get("fields", []):
                ...
                reference_value = self._lookup_golden_source(
                    source_name, kde_name, value, evidence
                )
    ...
    def _calculate_timeliness_score(
        self, evidence: Dict[str, Any], alert_timestamp: datetime = None
    ) -> float:
        ...
        for field in timestamp_fields:
            if field in evidence:
                try:
                    ...
                    delay_hours = (
                        alert_timestamp - field_timestamp
                    ).total_seconds() / 3600
    ...
    def _calculate_coverage_score(
        self, evidence: Dict[str, Any], baseline_data: Dict[str, Any] = None
    ) -> float:
        ...
        current_value = sum(
            float(v) for v in evidence.values() if isinstance(v, (int, float))
        )
        ...
        volume_drop = (
            max(0, (baseline_volume - current_volume) / baseline_volume * 100)
            if baseline_volume > 0
            else 0
        )
        value_drop = (
            max(0, (baseline_value - current_value) / baseline_value * 100)
            if baseline_value > 0
            else 0
        )
    ...
    def _aggregate_dqsi_score(
        self,
        kde_scores: Dict[str, Dict[str, float]],
        ...
        breakdown = {
            ...
            "foundational_weighted_score": (
                foundational_score / foundational_weights
                if foundational_weights > 0
                else 0.0
            ),
            "enhanced_weighted_score": (
                enhanced_score / enhanced_weights if enhanced_weights > 0 else 0.0
            ),
            "foundational_contribution": (
                (foundational_score / total_weighted_score * 100)
                if total_weighted_score > 0
                else 0.0
            ),
            "enhanced_contribution": (
                (enhanced_score / total_weighted_score * 100)
                if total_weighted_score > 0
                else 0.0
            ),
        }
    ...
    def _lookup_golden_source(
        self, source_name: str, kde_name: str, value: Any, evidence: Dict[str, Any]
    ) -> Any:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "role_kde_scope": {"analyst": ["trader_id", "notional", "price"]},
            "trust_bucket_thresholds": {"high": 0.85, "moderate": 0.65},
        }
