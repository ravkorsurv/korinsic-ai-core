...
class RiskCalculator:
    ...
    def __init__(self):
        self.risk_weights = {"insider_dealing": 0.6, "spoofing": 0.4}
        ...
        self.contextual_factors = {
            ...
            "volume_multipliers": {"high": 1.3, "medium": 1.1, "low": 1.0},
    ...
    def calculate_overall_risk(
        self, insider_score: Dict, spoofing_score: Dict, processed_data: Dict
    ) -> float:
        """Calculate overall market abuse risk score"""
        try:
            # Extract base risk scores
            insider_risk = (
                insider_score.get("overall_score", 0)
                if "error" not in insider_score
                else 0
            )
            spoofing_risk = (
                spoofing_score.get("overall_score", 0)
                if "error" not in spoofing_score
                else 0
            )
            ...
            logger.debug(
                f"Overall risk calculated: base={base_score:.3f}, "
    ...
    def _calculate_behavioral_multiplier(self, data: Dict) -> float:
        ...
        try:
            ...
            timing_concentration = data.get("metrics", {}).get(
    ...
    def calculate_risk_breakdown(
        ...
        try:
            ...
            breakdown["weighted_scores"]["spoofing"] = (
                breakdown["base_scores"]["spoofing"] * self.risk_weights["spoofing"]
            )
            ...
            breakdown["contextual_factors"]["trader_role"] = {
                "role": trader_role,
                "multiplier": self.contextual_factors["trader_role_multipliers"].get(
                    trader_role, 1.0
                ),
            }
            ...
            breakdown["contextual_factors"]["volume"] = {
                ...
                "multiplier": self.contextual_factors["volume_multipliers"].get(
                    volume_category, 1.0
                ),
            }
            ...
            breakdown["contextual_factors"]["timeframe"] = {
                "timeframe": timeframe,
                "multiplier": self.contextual_factors["timeframe_multipliers"].get(
                    timeframe, 1.0
                ),
            }

            breakdown["contextual_factors"]["market_conditions"] = {
                "multiplier": self._calculate_market_conditions_multiplier(
                    processed_data
                )
            }
    ...
    def calculate_confidence_interval(
        self, insider_score: Dict, spoofing_score: Dict
    ) -> Dict:
