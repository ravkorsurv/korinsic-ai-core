...
class EvidenceSufficiencyIndex:
    ...
    def calculate_esi(
        ...
        try:
            ...
            mean_confidence_score = self._calculate_mean_confidence_score(
                confidence_scores
            )
            ...
            cross_cluster_diversity = self._calculate_cross_cluster_diversity(
                node_states
            )
    ...
    def _calculate_mean_confidence_score(
    ...
    def adjust_risk_score(self, risk_score: float, esi_score: float) -> float:
        ...
        logger.info(
            f"Risk score adjusted: {risk_score:.3f} -> {adjusted_score:.3f} (ESI: {esi_score:.3f})"
        )
