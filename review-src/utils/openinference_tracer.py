...
class KorinsicOpenInferenceTracer:
    ...
    def _setup_tracing(self):
        ...
        try:
            ...
            self.tracer = trace.get_tracer(
                "korinsic.surveillance.platform", version="1.0.0"
            )
    ...
    def _setup_exporters(self):
        ...
        phoenix_endpoint = self.config.get(
    ...
    def trace_bayesian_inference(
        ...
        with self.tracer.start_as_current_span(
            f"bayesian_inference.{model_name}", kind=trace.SpanKind.INTERNAL
        ) as span:
            try:
                ...
                span.set_attribute(
                    "inference.start_time", datetime.utcnow().isoformat()
                )

                if input_data:
                    span.set_attribute(
                        "input.trade_count", len(input_data.get("trades", []))
                    )
                    span.set_attribute(
                        "input.order_count", len(input_data.get("orders", []))
                    )
                    span.set_attribute(
                        "input.has_material_events",
                        bool(input_data.get("material_events")),
                    )
