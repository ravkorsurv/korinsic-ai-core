...
def analyze_trading_data_enhanced():
    ...
    with tracer.tracer.start_as_current_span(
        "surveillance_analysis", kind=tracer.trace.SpanKind.SERVER
    ) as main_span:
        try:
            ...
            if not data:
                main_span.set_status(
                    tracer.trace.Status(
                        tracer.trace.StatusCode.ERROR, "No data provided"
                    )
                )
            ...
            include_regulatory_rationale = data.get(
                "include_regulatory_rationale", False
            )
            ...
            with tracer.tracer.start_as_current_span(
                ...
                processing_span.set_attribute(
                    "processing.use_latent_intent", use_latent_intent
                )
                ...
                processing_span.set_attribute(
                    "processing.processed_trade_count",
                    len(processed_data.get("trades", [])),
                )
                processing_span.set_attribute(
                    "processing.processed_order_count",
            ...
            if use_latent_intent:
                insider_result = enhanced_bayesian_engine.calculate_insider_dealing_risk_with_latent_intent(
                    processed_data
                )
            else:
                insider_result = (
                    enhanced_bayesian_engine.calculate_insider_dealing_risk(
                        processed_data
                    )
                )
            ...
            # Spoofing analysis
            spoofing_result = enhanced_bayesian_engine.calculate_spoofing_risk(
                processed_data
            )
            ...
            with tracer.tracer.start_as_current_span(
                ...
                aggregation_span.set_attribute(
                    "risk.insider_score", insider_result.get("overall_score", 0)
                )
                aggregation_span.set_attribute(
                    "risk.spoofing_score", spoofing_result.get("overall_score", 0)
                )
            ...
            response = {
                ...
                "data": {
                    ...
                    "tracing_metadata": {
                        ...
                        "trace_id": format(
                            main_span.get_span_context().trace_id, "032x"
                        ),
            ...
            main_span.set_status(
                tracer.trace.Status(tracer.trace.StatusCode.ERROR, str(e))
            )

            return (
                jsonify(
                    {"status": "error", "error": str(e), "analysis_id": analysis_id}
                ),
                500,
            )
...
def analyze_batch_enhanced():
    ...
    with tracer.tracer.start_as_current_span(
        "surveillance_batch_analysis", kind=tracer.trace.SpanKind.SERVER
    ) as batch_span:
        try:
            ...
            if not batch_data:
                batch_span.set_status(
                    tracer.trace.Status(
                        tracer.trace.StatusCode.ERROR, "No batch data provided"
                    )
                )
            ...
            for i, dataset in enumerate(batch_data):
                with tracer.tracer.start_as_current_span(
                    ...
                    item_span.set_attribute(
                        "batch.item.id", dataset.get("id", f"item_{i}")
                    )
            ...
            avg_risk = sum(
                result["risk_scores"]["overall_risk"] for result in results
            ) / len(results)

            batch_span.set_attribute("batch.average_risk", avg_risk)
            batch_span.set_attribute(
                "batch.total_high_risk_alerts", total_high_risk_alerts
            )
            ...
            response = {
                ...
                "data": {
                    ...
                    "tracing_metadata": {
                        "batch_trace_id": format(
                            batch_span.get_span_context().trace_id, "032x"
                        ),
                        "openinference_enabled": tracer.enabled,
                    },
                },
            ...
            logger.info(
                f"Batch analysis {batch_id} completed: {len(results)} datasets processed"
            )
            return jsonify(response)
            ...
            return (
                jsonify({"status": "error", "error": str(e), "batch_id": batch_id}),
                500,
            )
...
def _log_request_characteristics(span: Any, data: Dict[str, Any]):
    ...
    try:
        ...
        span.set_attribute(
            "request.has_material_events", bool(data.get("material_events"))
        )
        span.set_attribute(
            "request.use_latent_intent", data.get("use_latent_intent", False)
        )
        span.set_attribute(
            "request.include_regulatory_rationale",
            data.get("include_regulatory_rationale", False),
        )
...
def _calculate_overall_risk(
    insider_score: float, spoofing_score: float, processed_data: Dict[str, Any]
) -> float:
...
def _generate_regulatory_rationale(
    alert: Dict[str, Any], processed_data: Dict[str, Any]
) -> Dict[str, Any]:
