...
class ModelExplainabilityEngine:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.counterfactual_generator = CounterfactualGenerator(self.config.get('counterfactual', {}))
    ...
    def _generate_regulatory_summary(
        ...
        # Key factors
        key_factors = [fa.feature_name for fa in sorted(feature_attributions, key=lambda x: x.importance, reverse=True)[:3]]
    ...
    def _generate_decision_rationale(
        ...
        top_factors = sorted(feature_attributions, key=lambda x: abs(x.contribution), reverse=True)[:3]
        
        rationale_parts = []
        for factor in top_factors:
            direction = "increases" if factor.contribution > 0 else "decreases"
            rationale_parts.append(f"{factor.feature_name} {direction} risk by {abs(factor.contribution):.2%}")
    ...
    def _calculate_explainability_score(self, feature_attributions: List[FeatureAttribution]) -> float:
    ...
    def _generate_fallback_explanation(
        ...
        return {
            ...
            'decision_path': [],
            'uncertainty_analysis': asdict(UncertaintyAnalysis(
    ...
    def generate_scenarios(
        ...
        for i, (feature_name, value) in enumerate(evidence.items()):
            if isinstance(value, (int, float)) and i < 3:  # Max 3 scenarios
                ...
                counterfactual_prediction = original_prediction * (1 - (modified_value - value) / 10.0)
    ...
    def generate_decision_path(
        ...
        for i, (feature_name, value) in enumerate(evidence.items()):
            if isinstance(value, (int, float)):
                ...
                decision_path.append(DecisionPathNode(
...
class UncertaintyQuantifier:
    ...
    def analyze_uncertainty(
        ...
        evidence_quality = len([v for v in evidence.values() if v is not None]) / len(evidence) if evidence else 0.0
