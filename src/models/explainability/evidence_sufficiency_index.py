...
class EvidenceSufficiencyIndex:
    ...
    def calculate_esi(self, evidence: Dict[str, Any], result: Dict[str, Any]) -> ESIResult:
        ...
        try:
            ...
            esi = (
                self.weights["W1"] * node_activation_ratio
    ...
    def generate_esi_explanation(self, esi_result: ESIResult) -> str:
        ...
        cluster_info = (
            f"spanning {len(esi_result.clusters)} evidence clusters"
            if esi_result.clusters
            else "from limited sources"
        )

        return (
            f"ESI: {quality} Evidence ({esi_score}) - This alert is supported by "
