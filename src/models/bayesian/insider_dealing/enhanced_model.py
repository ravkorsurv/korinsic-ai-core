...
class EnhancedInsiderDealingModel(EnhancedBaseModel):
    ...
    def __init__(self, use_latent_intent: bool = False, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"Enhanced insider dealing model initialized (latent_intent={use_latent_intent})")
    ...
    def calculate_risk_with_explanation(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            enhanced_result = {
                ...
                'evidence_sufficiency': core_risk_result.get('evidence_sufficiency', {}),
            ...
            if self.governance_enabled:
                self.governance_tracker.track_model_lifecycle(
                    ...
                    metadata={
                        'risk_level': enhanced_result.get('risk_assessment', {}).get('risk_level', 'unknown'),
                        'evidence_quality': validation_result.get('evidence_quality_score', 0.0),
                                                 'explainability_score': 0.0 if not isinstance(explanation, dict) else explanation.get('explanation_metadata', {}).get('explanation_quality', 0.0)
                    }
                )
            ...
            logger.error(f"Error in enhanced insider dealing risk calculation: {str(e)}")
            raise
    ...
    def generate_counterfactuals(self, evidence: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            baseline_score = baseline_result.get('risk_scores', {}).get('overall_score', 0.0)
            ...
            if evidence.get('MaterialInfo', 0) > 0:
                ...
                modified_score = modified_result.get('risk_scores', {}).get('overall_score', 0.0)
    ...
    def explain_decision_path(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            decision_path['decision_steps'].append({
                ...
                'output': {
                    'level': ['No access', 'Potential access', 'Clear access'][min(material_info, 2)],
            ...
            decision_path['decision_steps'].append({
                ...
                'output': {
                    'level': ['Normal', 'Unusual', 'Highly unusual'][min(trading_activity, 2)],
                    'weight': 0.3,
            ...
            decision_path['decision_steps'].append({
                ...
                'output': {
                    ...
                    'risk_level': risk_result.get('risk_assessment', {}).get('risk_level', 'unknown'),
                    'confidence': risk_result.get('risk_scores', {}).get('confidence', 'medium')
                },
            ...
            decision_path['final_assessment'] = {
                ...
                'regulatory_rationale': self._generate_regulatory_rationale(evidence, overall_score)
            }
    ...
    def _generate_insider_dealing_specifics(
        ...
        return {
            ...
            'regulatory_assessment': {
                'prosecution_likelihood': self._assess_prosecution_likelihood(risk_result, evidence),
                'regulatory_action_probability': self._assess_regulatory_action_probability(risk_result),
                'compliance_recommendation': self._generate_compliance_recommendation(risk_result, evidence)
            }
        }
    ...
    def _generate_regulatory_rationale(self, evidence: Dict[str, Any], risk_score: float) -> str:
        """Generate regulatory rationale for the decision."""
        
        if risk_score >= 0.8:
            return ("High probability of insider dealing based on strong evidence of material information "
            ...
            return ("Moderate probability of insider dealing with some concerning indicators. "
                   "Enhanced monitoring and further investigation recommended.")
        elif risk_score >= 0.3:
            return ("Low to moderate probability of insider dealing with limited indicators. "
                   "Continued monitoring recommended.")
        else:
            return ("No significant indicators of insider dealing detected. "
    ...
    def _check_mar_compliance(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'article_14_elements': {
                ...
                'price_sensitive': evidence.get('PriceImpact', 0) >= 1 if 'PriceImpact' in evidence else True
            },
            'compliance_met': all([
    ...
    def _assess_prosecution_likelihood(self, risk_result: Dict[str, Any], evidence: Dict[str, Any]) -> str:
    ...
    def _generate_compliance_recommendation(self, risk_result: Dict[str, Any], evidence: Dict[str, Any]) -> str:
        """Generate compliance recommendation."""
        
        risk_score = risk_result.get('risk_scores', {}).get('overall_score', 0.0)
        
        if risk_score >= 0.8:
            return ("Immediate action required: File suspicious transaction report (STR), "
                   "conduct internal investigation, preserve evidence, consider trade restrictions.")
        elif risk_score >= 0.6:
            return ("Enhanced monitoring required: Increase surveillance frequency, "
                   "gather additional evidence, prepare preliminary documentation.")
        elif risk_score >= 0.3:
            return ("Continued monitoring: Maintain current surveillance level, "
