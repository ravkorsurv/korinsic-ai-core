...
class Config:
    ...
    def _load_config_file(self, filename: str) -> Dict[str, Any]:
        ...
        if not config_path.exists():
            if filename == "base.json":
                ...
                logger.warning(
                    f"Environment config file not found: {config_path}, using base only"
                )
                return {}
    ...
    def _apply_env_overrides(self):
        ...
        env_overrides = {
            ...
            "INSIDER_MEDIUM_THRESHOLD": (
                "risk_thresholds.insider_dealing.medium",
                float,
            ),
        ...
        for env_var, (config_path, converter) in env_overrides.items():
            ...
            if env_value is not None:
                try:
                    ...
                    logger.warning(
                        f"Failed to convert env var {env_var}={env_value}: {e}"
                    )
    ...
    def get_model_config_path(self) -> str:
        ...
        return str(
            self.config_dir
            / self.get("models.model_config_path", "models/bayesian_models.json")
        )
