...
class DQConfig:
    ...
    # Tier weights
    dimension_tier_weights: Dict[str, float] = field(
        default_factory=lambda: {"foundational": 1.0, "enhanced": 0.75}
    )

    # KDE risk tiers
    kde_risk_tiers: Dict[str, str] = field(
        default_factory=lambda: {
            "trader_id": "high",
            "trade_time": "high",
    ...
    critical_kdes: List[str] = field(
        default_factory=lambda: ["trader_id", "order_timestamp"]
    )
    dqsi_critical_cap: float = 0.75

    # Synthetic KDE settings
    synthetic_kdes: Dict[str, Dict[str, Any]] = field(
        default_factory=lambda: {
            "timeliness": {
                "weight": 3,
    ...
    confidence_params: Dict[str, float] = field(
        default_factory=lambda: {
...
class DQScoringStrategy(ABC):
    ...
    def calculate_confidence_index(
        self, kde_results: List[KDEResult], dqsi_score: float
    ) -> Tuple[float, str]:
        """
        ...
        confidence_note = (
            f"{critical_missing} high-risk KDE missing, {imputation_rate:.0%} imputed"
        )
        if self.get_strategy_name() == "fallback":
    ...
    def calculate_dimension_scores(
        self, kde_results: List[KDEResult]
    ) -> Dict[str, float]:
    ...
    def _calculate_timeliness_score(
        self, data: Dict[str, Any], metadata: Optional[Dict[str, Any]]
    ) -> float:
    ...
    def _calculate_coverage_score(
        ...
        if metadata and "baseline_volume" in metadata:
            ...
            if baseline_volume > 0:
                drop_percentage = max(
                    0, (baseline_volume - current_volume) / baseline_volume
                )
