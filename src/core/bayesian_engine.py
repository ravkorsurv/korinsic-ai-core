...
def map_insider_dealing_evidence(processed_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    evidence["trade_pattern"] = (
        1
        if market_data.get("volatility", 0) > 0.2
        or market_data.get("price_movement", 0) > 0.1
        else 0
    )
    # comms_intent: 0=benign, 1=suspicious, 2=malicious
    evidence["comms_intent"] = (
        2 if any("sensitive" in c.get("content", "").lower() for c in comms) else 0
    )
    ...
    evidence["state_information_access"] = (
        2 if trader_info.get("access_level", "").lower() == "high" else 0
    )
    logger.info(f"[EVIDENCE MAPPING] mapped_evidence={evidence}")
    return evidence


def apply_fallback_evidence_with_usage(
...
class BayesianEngine:
    ...
    def _create_insider_dealing_model(self):
        ...
        self.insider_dealing_node_defs = {
            "trade_pattern": BayesianNode(
                "trade_pattern", ["normal", "suspicious"], fallback_prior=[0.95, 0.05]
            ),
            "comms_intent": BayesianNode(
                "comms_intent",
                ["benign", "suspicious", "malicious"],
                fallback_prior=[0.8, 0.15, 0.05],
            ),
            "pnl_drift": BayesianNode(
                "pnl_drift", ["normal", "anomalous"], fallback_prior=[0.9, 0.1]
            ),
            "news_timing": BayesianNode(
                "news_timing",
                ["normal", "delayed", "unexplained"],
                fallback_prior=[0.85, 0.12, 0.03],
            ),
            "state_information_access": BayesianNode(
                "state_information_access",
                ["none", "potential", "clear"],
    ...
    def _create_spoofing_model(self):
        ...
        cpd_cancellation = TabularCPD(
            ...
            variable_card=3,
            values=[[0.8, 0.4, 0.1], [0.15, 0.4, 0.3], [0.05, 0.2, 0.6]],
        ...
        cpd_volume = TabularCPD(
            ...
            variable_card=3,
            values=[[0.8, 0.5, 0.2], [0.15, 0.3, 0.3], [0.05, 0.2, 0.5]],
            evidence=["OrderPattern"],
            evidence_card=[3],
        )
        ...
        cpd_risk = TabularCPD(
            variable="Risk",
            variable_card=3,
            values=[base_low * 3, base_med * 3, base_high * 3],
            evidence=[
                "OrderPattern",
                "CancellationRate",
                "PriceMovement",
        ...
        self.spoofing_node_defs = {
            "OrderPattern": BayesianNode(
                "OrderPattern",
                ["benign", "suspicious", "malicious"],
                fallback_prior=[0.85, 0.12, 0.03],
            ),
            "CancellationRate": BayesianNode(
                "CancellationRate",
                ["low", "medium", "high"],
                fallback_prior=[0.8, 0.15, 0.05],
            ),
            "PriceMovement": BayesianNode(
                "PriceMovement",
                ["stable", "volatile", "extreme"],
                fallback_prior=[0.7, 0.25, 0.05],
            ),
            "VolumeRatio": BayesianNode(
                "VolumeRatio",
                ["low", "medium", "high"],
                fallback_prior=[0.8, 0.15, 0.05],
            ),
    ...
    def analyze_insider_dealing(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            evidence, fallback_usage = apply_fallback_evidence_with_usage(
                mapped_evidence, self.insider_dealing_node_defs
            )
            # Perform inference
            query = self.insider_dealing_inference.query(
                variables=["insider_dealing"], evidence=evidence
            )
            risk_probabilities = query.values
            # Calculate risk score as probability of 'yes' (index 1)
            risk_score = (
                float(risk_probabilities[1])
                if hasattr(risk_probabilities, "__getitem__")
                else 0.0
            )
            logger.info(
                f"[BAYESIAN] risk_probabilities={risk_probabilities}, risk_score={risk_score}"
            )
            # Calculate ESI
            esi_score = self.esi_calculator.calculate_esi(
                evidence, evidence, fallback_usage
            )
            return {
                ...
                "risk_probabilities": (
                    ...
                    if hasattr(risk_probabilities, "tolist")
                    else list(risk_probabilities)
                ),
                "esi_score": esi_score,
