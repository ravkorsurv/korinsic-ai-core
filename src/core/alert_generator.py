...
from .regulatory_explainability import (
    RegulatoryExplainability,
    RegulatoryRationale,
    STORRecord,
)
...
class AlertGenerator:
    ...
    def __init__(self):
        self.alert_thresholds = {
            "insider_dealing": {"high_risk": 0.6, "medium_risk": 0.3},
            "spoofing": {"high_risk": 0.8, "medium_risk": 0.5},
            "overall_risk": {"critical": 0.8, "high": 0.6, "medium": 0.4},
        }
    ...
    def generate_alerts(
        ...
        logger.info(
            f"[ALERT] generate_alerts called with insider_score={insider_score}, spoofing_score={spoofing_score}, overall_risk={overall_risk}"
        )
        alerts = []
        try:
            ...
            for alert in alerts:
                ...
                logger.warning(
                    f"ALERT GENERATED: {alert['type']} - {alert['severity']}"
                )
    ...
    def _check_insider_dealing_alerts(self, data: Dict, scores: Dict) -> List[Dict]:
        ...
        if overall_score >= thresholds["high_risk"]:
            ...
            logger.info(
                f"[ALERT] Insider dealing risk score {overall_score:.3f} below threshold; no alert."
            )
            return alerts
        logger.info(
            f"[ALERT] Insider dealing risk score {overall_score:.3f} triggers {severity} alert."
        )
    ...
    def _generate_spoofing_description(self, data: Dict, scores: Dict) -> str:
        ...
        critical_nodes = scores.get("critical_nodes", [])
        description = (
            f"Potential spoofing activity detected with {orders_count} orders. "
        )
        if news_context == 0:
            description += "(Move explained by public news/event.) "
        elif news_context == 1:
    ...
    def _compile_spoofing_evidence(self, data: Dict, scores: Dict) -> Dict:
        ...
        return {
            ...
            "large_orders_count": len(
                [o for o in data.get("orders", []) if o.get("size", 0) > 10000]
            ),
            "cancelled_orders_count": len(
                [o for o in data.get("orders", []) if o.get("status") == "cancelled"]
            ),
            "news_context": scores.get("news_context", None),
            "high_nodes": scores.get("high_nodes", []),
            "critical_nodes": scores.get("critical_nodes", []),
    ...
    def get_historical_alerts(
        ...
        if alert_type:
            filtered_alerts = [
                a for a in filtered_alerts if a["type"] == alert_type.upper()
            ]
    ...
    def generate_regulatory_rationale(
        self, alert: Dict, risk_scores: Dict, processed_data: Dict
    ) -> RegulatoryRationale:
    ...
    def export_stor_report(
        self, alert: Dict, risk_scores: Dict, processed_data: Dict
    ) -> STORRecord:
        """Export alert in STOR format"""
        try:
            rationale = self.generate_regulatory_rationale(
                alert, risk_scores, processed_data
            )
            return self.regulatory_explainability.export_stor_format(
                rationale, processed_data
            )
        except Exception as e:
