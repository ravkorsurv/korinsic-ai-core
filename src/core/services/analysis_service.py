...
class AnalysisService:
    ...
    def analyze_trading_data(
        self, data: Dict[str, Any], use_latent_intent: bool = False
    ) -> AnalysisResult:
        """
        ...
        try:
            ...
            if use_latent_intent:
                insider_dealing_score = self.bayesian_engine.calculate_insider_dealing_risk_with_latent_intent(
                    processed_data
                )
            else:
                insider_dealing_score = (
                    self.bayesian_engine.calculate_insider_dealing_risk(processed_data)
                )

            spoofing_score = self.bayesian_engine.calculate_spoofing_risk(
                processed_data
            )
            ...
            logger.info(
                f"Analysis {analysis_id} completed in {processing_time_ms:.2f}ms"
            )
            return result
    ...
    def analyze_realtime_data(self, data: Dict[str, Any]) -> AnalysisResult:
        ...
        try:
            ...
            insider_dealing_score = self.bayesian_engine.calculate_insider_dealing_risk(
                processed_data
            )
            spoofing_score = self.bayesian_engine.calculate_spoofing_risk(
                processed_data
