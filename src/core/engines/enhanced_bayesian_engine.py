...
class EnhancedBayesianEngine(BayesianEngine):
    ...
    def __init__(self, tracing_config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Enhanced Bayesian engine initialized with session {self.session_id}"
        )
    ...
    def calculate_insider_dealing_risk(
        ...
        with self.tracer.trace_bayesian_inference(
            ...
            try:
                ...
                if span:
                    self._log_input_characteristics(
                        span, processed_data, "insider_dealing"
                    )
                ...
                logger.info(
                    f"Insider dealing risk calculated in {processing_time:.3f}s"
                )
    ...
    def calculate_spoofing_risk(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        with self.tracer.trace_bayesian_inference(
            model_name="spoofing", model_version="1.0.0", input_data=processed_data
        ) as span:
            try:
                # Log input characteristics
                if span:
    ...
    def calculate_insider_dealing_risk_with_latent_intent(
        ...
        with self.tracer.trace_bayesian_inference(
            ...
            try:
                ...
                if span:
                    self._log_input_characteristics(
                        span, processed_data, "insider_dealing_latent_intent"
                    )
                    span.set_attribute("model.latent_intent", True)

                # Perform the actual risk calculation
                start_time = datetime.utcnow()
                risk_result = super().calculate_insider_dealing_risk_with_latent_intent(
                    processed_data
                )
                end_time = datetime.utcnow()
                ...
                logger.info(
                    f"Insider dealing risk (latent intent) calculated in {processing_time:.3f}s"
                )
                return enhanced_result

            except Exception as e:
                logger.error(
                    f"Error in latent intent insider dealing risk calculation: {e}"
                )
                if span:
                    span.record_exception(e)
                raise
    ...
    def _log_input_characteristics(
        self, span: Any, processed_data: Dict[str, Any], model_type: str
    ):
        """
        ...
        try:
            ...
            if trades:
                timestamps = [
                    trade.get("timestamp") for trade in trades if trade.get("timestamp")
                ]
                if timestamps:
                    span.set_attribute(
    ...
    def _log_prediction_results(
        self, span: Any, result: Dict[str, Any], processing_time: float
    ):
        """
        ...
        try:
            ...
            if "evidence_sufficiency_index" in result:
                span.set_attribute(
                    "prediction.evidence_sufficiency",
            ...
            if "contributing_factors" in result:
                ...
                span.set_attribute(
                    "prediction.contributing_factors_count", len(factors)
                )
                if factors:
                    factor_names = [factor.get("name", "unknown") for factor in factors]
                    span.set_attribute(
                        "prediction.contributing_factors", ",".join(factor_names)
                    )
