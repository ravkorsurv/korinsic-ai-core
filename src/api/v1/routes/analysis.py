...
def analyze_trading_data():
    ...
    try:
        ...
        analysis_result = analysis_service.analyze_trading_data(
            data, use_latent_intent=use_latent_intent
        )
        ...
        logger.info(
            f"Analysis completed for {len(analysis_result.processed_data.get('trades', []))} trades"
        )
    ...
    try:
        ...
        response = {
            ...
            "summary": {
                ...
                "total_alerts": sum(
                    len(result.get("alerts", [])) for result in batch_results
                ),
            },
        }
