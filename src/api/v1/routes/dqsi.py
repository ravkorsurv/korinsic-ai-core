...
def calculate_dqsi_score():
    ...
    try:
        ...
        if config.role_aware:
            ...
            enhanced_results = dqsi_calculator.calculate_dqsi_enhanced(
                processed_data, dimension_configs
            )
            ...
            if include_recommendations:
                response["recommendations"] = _get_enhanced_recommendations(
                    enhanced_results
                )
        else:
            ...
            recommendations = []
            if include_recommendations:
                recommendations = dqsi_calculator.get_improvement_recommendations(
                    metrics
                )

            # Build response
            response = DQSIResponseSchema().build_response(
                metrics=metrics, report=report, recommendations=recommendations
            )
            response["calculation_type"] = "traditional"

        # Log results
        overall_score = (
            enhanced_results.get("overall_score", 0.0)
            if config.role_aware
            else metrics.overall_score
        )
        logger.info(
            f"DQSI calculated: {overall_score:.3f} for dataset with {_get_data_size(processed_data)} records"
        )
    ...
    try:
        ...
        response = {
            ...
            "summary": {
                ...
                "average_dqsi": sum(r["dqsi_score"] for r in batch_results)
                / len(batch_results),
                "best_performer": max(batch_results, key=lambda x: x["dqsi_score"])[
                    "dataset_id"
                ],
                "worst_performer": min(batch_results, key=lambda x: x["dqsi_score"])[
                    "dataset_id"
                ],
            },
...
def monitor_dqsi_trends():
    ...
    try:
        ...
        alerting_thresholds = data.get(
            "alerting_thresholds", {"critical": 0.4, "warning": 0.6, "target": 0.8}
        )
        ...
        for entry in time_series_data:
            ...
            # Calculate DQSI metrics
            metrics = dqsi_calculator.calculate_dqsi(
                processed_data, data.get("dimension_configs", {})
            )
            ...
            time_series_results.append(
                {
                    ...
                    "dqsi_score": metrics.overall_score,
                    "dimension_scores": metrics.dimension_scores,
                    "status": dqsi_calculator._get_overall_status(
        ...
        trend_analysis = _generate_trend_analysis(
            time_series_results, trend_analysis_window
        )
        ...
        response = {
            ...
            "summary": {
                ...
                "current_score": (
                    time_series_results[-1]["dqsi_score"]
...
def validate_data_quality():
    ...
    try:
        ...
        # Extract request parameters
        dataset = data.get("dataset", {})
        validation_thresholds = data.get(
...
def _generate_comparison_analysis(
    ...
    try:
        ...
        highest_variance_dim = (
            max(dimension_variances.keys(), key=lambda x: dimension_variances[x])
        ...
        return {
            ...
            "insights": {
                ...
                "performance_consistency": (
                    "high" if std_dev < 0.1 else "medium" if std_dev < 0.2 else "low"
                ),
                "overall_quality_level": (
                    "excellent"
                    if avg_score >= 0.9
                    else "good" if avg_score >= 0.8 else "needs_improvement"
                ),
            },
        }
...
def _generate_trend_analysis(
    time_series_results: List[Dict[str, Any]], window: int
) -> Dict[str, Any]:
...
def _generate_dqsi_alerts(
    time_series_results: List[Dict[str, Any]], thresholds: Dict[str, float]
) -> List[Dict[str, Any]]:
...
def _get_enhanced_recommendations(
    ...
    try:
        ...
        for dimension, result in dimension_results.items():
            ...
            if score < 0.8:
                ...
                rec = {
                    ...
                    "suggestions": _get_role_aware_suggestions(
                        dimension, score, role, quality_level
                    ),
                }

                # Add sub-dimension specific recommendations
                if hasattr(result, "sub_dimensions") and getattr(
                    result, "sub_dimensions", None
                ):
        ...
        recommendations.sort(
            key=lambda x: (x["priority"] == "high", -x["current_score"])
        )
...
def _get_role_aware_suggestions(
    dimension: str, score: float, role: str, quality_level: str
) -> List[str]:
    ...
    if role == "producer":
        ...
        if quality_level == "enhanced":
            base_suggestions[dimension].extend(
                ["Add trend analysis monitoring", "Implement cross-system validation"]
            )
        else:
            base_suggestions[dimension].extend(
                [
